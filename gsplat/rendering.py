...
def rasterization_radegs(
    ...
    if colors.shape[-1] > channel_chunk:
        # slice into chunks
        n_chunks = (colors.shape[-1] + channel_chunk - 1) // channel_chunk
        render_colors, render_alphas, render_depths, render_normals = [], [], [], []
        for i in range(n_chunks):
            colors_chunk = colors[..., i * channel_chunk : (i + 1) * channel_chunk]
            backgrounds_chunk = (
                backgrounds[..., i * channel_chunk : (i + 1) * channel_chunk]
            ...
            render_colors_, render_alphas_, render_depths_, render_normals_ = rasterize_to_pixels_radegs(
                means2d,
                conics,
                colors_chunk,
                opacities,
